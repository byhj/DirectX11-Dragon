...
class LitSkullApp :public D3DApp
{
	...
	private:
		...
		void init_buffer2();
	private:
		ID3D11Buffer                * pShapesVB;
		ID3D11Buffer                * pShapesIB;
...
LitSkullApp::LitSkullApp(HINSTANCE hInstance)
	:D3DApp(hInstance),
    pShapesVB(0), pShapesIB(0), pSkullVB(0), pSkullIB(0), mSkullIndexCount(0), mLightCount(1),
	mEyePosW(0.0f, 0.0f, 0.0f), mTheta(1.5f*MathHelper::Pi), mPhi(0.1f*MathHelper::Pi), mRadius(15.0f)
{
	WindowTitle = L"LitSkull Demo";
...
{
	ReleaseCOM(pShapesVB);
	ReleaseCOM(pShapesIB);
	ReleaseCOM(pSkullVB);
	ReleaseCOM(pSkullIB);
...
{
	pDeviceContext->ClearRenderTargetView(pRenderTargetView, reinterpret_cast<const float*>(&Colors::LightSteelBlue));
	pDeviceContext->ClearDepthStencilView(pDepthStencilView, D3D11_CLEAR_DEPTH|D3D11_CLEAR_STENCIL, 1.0f, 0);

	pDeviceContext->IASetInputLayout(InputLayouts::PosNormal);
	...
	{
		pDeviceContext->IASetVertexBuffers(0, 1, &pShapesVB, &stride, &offset);
		pDeviceContext->IASetIndexBuffer(pShapesIB, DXGI_FORMAT_R32_UINT, 0);
		...
		pDeviceContext->DrawIndexed(mGridIndexCount, mGridIndexOffset, mGridVertexOffset);
		...
		activeTech->GetPassByIndex(p)->Apply(0, pDeviceContext);
		pDeviceContext->DrawIndexed(mBoxIndexCount, mBoxIndexOffset, mBoxVertexOffset);
		...
		for(int i = 0; i < 10; ++i)
		{
			...
			activeTech->GetPassByIndex(p)->Apply(0, pDeviceContext);
			pDeviceContext->DrawIndexed(mCylinderIndexCount, mCylinderIndexOffset, mCylinderVertexOffset);
		}
		...
		for(int i = 0; i < 10; ++i)
		{
			...
			activeTech->GetPassByIndex(p)->Apply(0, pDeviceContext);
			pDeviceContext->DrawIndexed(mSphereIndexCount, mSphereIndexOffset, mSphereVertexOffset);
		}

		// Draw the skull.

		pDeviceContext->IASetVertexBuffers(0, 1, &pSkullVB, &stride, &offset);
		pDeviceContext->IASetIndexBuffer(pSkullIB, DXGI_FORMAT_R32_UINT, 0);
		...
		activeTech->GetPassByIndex(p)->Apply(0, pDeviceContext);
		pDeviceContext->DrawIndexed(mSkullIndexCount, 0, 0);
	}
...
void LitSkullApp::init_buffer()
{
	...
	HR(pDevice->CreateBuffer(&vbd, &vinitData, &pShapesVB));
	...
	HR(pDevice->CreateBuffer(&ibd, &iinitData, &pShapesIB));
}
void LitSkullApp::init_buffer2()
{
	...
	D3D11_SUBRESOURCE_DATA vinitData;
	vinitData.pSysMem = &vertices[0];
	HR(pDevice->CreateBuffer(&vbd, &vinitData, &pSkullVB));
	...
	D3D11_SUBRESOURCE_DATA iinitData;
	iinitData.pSysMem = &indices[0];
	HR(pDevice->CreateBuffer(&ibd, &iinitData, &pSkullIB));
