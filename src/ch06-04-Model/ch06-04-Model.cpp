...
public:
	...
	void v_Render()
	{
		...
		m_model.Render(m_pD3D11DeviceContext, Model, View, Proj);
...
{
	m_camera.Init(20.0f, GetAspect());

	m_timer.Reset();

	m_font.init(m_pD3D11Device, L"Arial");

	m_model.init_buffer(m_pD3D11Device, m_pD3D11DeviceContext);
	m_model.init_shader(m_pD3D11Device, m_hWnd);
}
